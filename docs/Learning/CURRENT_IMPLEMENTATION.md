...
don't trust the table blindly. Last updated: end of Phase 4.
...
| Credentials | Plaintext `UserDefaults`, singleton `AuthManager`; now conforms to `AuthHeaderProviding` so `AuthenticationInterceptor` depends on the protocol, not `.shared` | `Assignment/Utils/AuthManager.swift`, `Assignment/Services/Remote/Interceptors.swift` | Indirectly via `RemoteServiceTests.requestInterceptorHeadersReachTheFinalRequest` (using a fake provider) | Passing | Still plaintext storage, still a singleton class underneath (Phase 5) |
...
| Testing | `AssignmentTests` (Swift Testing, 59 tests) + `AssignmentUITests` (XCTest, 1 smoke test) | `AssignmentTests/*.swift`, `AssignmentTests/Fakes/*.swift`, `AssignmentUITests/AssignmentUITests.swift` | 60 tests total, all passing | Passing | No actor/UIKit-bridge tests yet — those arrive with Phases 5-6 |
...
```
Result: **TEST SUCCEEDED**, 60/60 tests passing, 0 warnings (excluding the
